...
- **No behavioural analytics.** No Mixpanel, Amplitude, Segment, or similar, and no advertising or social pixels. Page-view counting comes from Vercel, the host, which is cookieless, assigns no visitor identifier, and never receives lesson content or progress. Everything else flows only to our own Supabase project. `app/privacy/page.tsx` states this in plain language and must be kept true.
